...
- Setups A–C and local Spec 01 are complete. Setup B provides local frontend, backend health, and disposable database workflows; Setup C deployed only a public preview and health endpoint and published the Google External OAuth app. Spec 01 adds a local catalog schema and card view; the development page can show six reviewed factual offerings, while tests use synthetic fixtures. No recommendation flow or real sign-in is implemented yet. The launch catalog remains unverified. Inspect the tracker and recorded evidence before claiming feature or deployment readiness.
